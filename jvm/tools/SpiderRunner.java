...
public class SpiderRunner {
  ...
  public static void main(String[] args) {
    // ★★ 关键修复：结果通道 = 进程 stdout，必须从**第一行**起就干净 ★★
    //
    //   现象：DouDou / NanGua / PanSso / PanSearch / YiSo / Push / MIPanSo / KkSs 等源
    //         明明能实例化，宿主却只收到一行中文（"自定义爬虫代码加载成功！"）或半截日志，
    //         被当成"蜘蛛返回了非法 JSON"。
    //
    //   根因：这批蜘蛛（以及它们的基类 com.github.catvod.spider.Pan）在**构造函数**里
    //         就有调试打印，且打印内容来自混淆字符串解码（字面量不在 DEX 字符串表里，
    //         反汇编时既搜不到、又必然在运行时输出）：
    //
    //             Pan.<init>:  const-string 'VLm4PVv8Y3YyAjX7bm7M33W'
    //                          invoke-static merge.<X>.decode(...)
    //                          sget-object System.out ; PrintStream.println
    //
    //   之前把 System.setOut(System.err) 只放在"分发调用"外面 —— 太晚了：
    //   类加载（Class.forName(..., true, cl) 触发 <clinit>）、newInstance()（<init>）、
    //   init(Context,ext) 三个阶段都已经把噪音写进了真 stdout。
    //
    //   修法：main() 第一件事就把 System.out 指向 stderr，全程隔离，最后只用
    //         恢复后的真 PrintStream 输出**唯一**一行结果 JSON。
    ...
    try {
      ...
      // 7) 分发调用
      //
      //   stdout 已在 main() 开头整体重定向到 stderr（见顶部注释），这里只需正常调用，
      //   最后用真 PrintStream 输出唯一一行结果。
      String result;
