...
- **单仓 JSON**（含 `sites` / `parses` / `lives`）—— 网络地址或本机 `.json` 路径。
- **多仓索引**（含 `urls[]`）—— 自动递归拉取并合并子仓；子仓里的 `spider` 字段也会被继承。

程序对「网上随便扒来的乱源」做了兜底，不必手工改 JSON：

- **顶层源 + 子仓源合并共存**：旧版带 `urls[]` 时会用子仓结果覆盖掉顶层自带源，子仓大面积失效就导致空白；现在顶层源始终保留。
- **多仓子仓有界并发抓取**：同一层最多 16 个并发、单链接 15 秒超时，失效链接最多等 15 秒整批跳过，导入大体积多仓配置不再假死。
- **坏源自动剔除**：合并时跳过畸形条目、缺 `key`/`name` 的条目、重复 `key`；单源解析失败 / 不支持的蜘蛛 / 聚合搜索中某个源挂了，都不会拖垮整体。
